...
out = train_set[:, -3].astype(np.float32)  # Batch x 2, taking the specified columns

# Validation Data
inp_val = val_set[:, :-3].astype(np.float32)  # Batch x (No. of features - 3)
out_val = val_set[:, -3].astype(np.float32)  # Batch x 2, taking the specified columns
...
num_features = 200
depth = 3
weight_decay = 3.5135415800967294e-05
learning_rate = 0.0008787352592162626
...
torch.save(model.state_dict(), "cf_model.pth")
